...
func DecodeMethodBidirectionalStreamingRPCWithPayloadRequest(ctx context.Context, v interface{}, md metadata.MD) (interface{}, error) {
	...
	var (
		message *pb.MethodBidirectionalStreamingRPCWithPayloadRequest
		ok      bool
	)
	{
		if message, ok = v.(*pb.MethodBidirectionalStreamingRPCWithPayloadRequest); !ok {
			return nil, goagrpc.ErrInvalidType("ServiceBidirectionalStreamingRPCWithPayload", "MethodBidirectionalStreamingRPCWithPayload", "*pb.MethodBidirectionalStreamingRPCWithPayloadStreamingRequest", v)
		}
	}
	var (
	...
	{
		payload = NewPayload(message, a, b)
	}
	return payload, err
}
`
