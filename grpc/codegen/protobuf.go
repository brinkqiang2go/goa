...
func (p *protobufAnalyzer) Dup(att *expr.AttributeExpr, required bool) codegen.AttributeAnalyzer {
	return &protobufAnalyzer{
		Analyzer: p.Analyzer.Dup(att, required).(*codegen.Analyzer),
	}
}
...
func protoBufGoFullTypeName(att *expr.AttributeExpr, pkg string, s *codegen.NameScope) string {
	...
	case *expr.Object:
		return s.GoTypeDef(codegen.NewAttributeAnalyzer(att, false, false, false, false, pkg, s))
	default:
